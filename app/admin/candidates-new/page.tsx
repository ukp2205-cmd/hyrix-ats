...
export default function AdminNewCandidatePage() {
  ...
  // Load user email and role from localStorage
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchOrganizationLogo = async () => {
      ...
      const { data: rows } = await pgFetch('select', 'organization', {
        select: 'logo_url',
        filters: [{ column: 'email', op: '=', value: userEmail }],
      })
      const orgData = Array.isArray(rows) ? rows[0] : rows
      if (orgData?.logo_url) setOrganizationLogo(orgData.logo_url)
    }
          ...
          <div className="p-4 border-t space-y-3">
            {isSidebarCollapsed ? (
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex justify-center" title={userEmail || 'admin@hyrix.ai'}>
                  {organizationLogo ? (
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-5xl mx-auto px-3 py-2">
            <Suspense fallback={<div className="p-4">Loading...</div>}>
              <AddCandidateForm 
                key="admin-candidate-form"
                userRole={userRole} 
                redirectPath="/admin?tab=candidates"
              />
            </Suspense>
          </div>
        </div>
